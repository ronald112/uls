...
static void swap_cat_ifl_true(t_catalog *a, t_catalog *b) {
    ...
    mx_swap_ll(&a->max_lnght_namedir, &b->max_lnght_namedir);
    mx_swap_ll(&a->max_lnght_grpdir, &b->max_lnght_grpdir);
    mx_swap_ll(&a->lng_max_minor, &b->lng_max_minor);
    mx_swap_ll(&a->lng_max_major, &b->lng_max_major);
}
...
void mx_swap_cat(t_catalog *a, t_flag flag, t_catalog *b) {
    ...
    b->dir_data = tmp_data;
    mx_swap_ll(&a->am_files, &b->am_files);
    mx_swap_ll(&a->am_data, &b->am_data);
