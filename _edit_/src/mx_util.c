...
char *mx_get_hex_view(int nmb) {
    ...
    result[1] = 'x';
    for (int i = 10 - mx_strlen(nmb_char); i < 10; ++nmb_digits) {
...
void mx_add_indens_minor_major(t_catalog *cat, t_dir_data *list) {
    if (cat->lng_max_minor
        ...
        cat->lng_max_minor = mx_get_nmb_digits_int(
                           MX_MINOR(list->buff_stat->st_rdev));
    }
    if (cat->lng_max_major
        < mx_get_nmb_digits_int(MX_MAJOR(list->buff_stat->st_dev))) {
        cat->lng_max_major = mx_get_nmb_digits_int(
                           MX_MAJOR(list->buff_stat->st_rdev));
    }
}
